...
pub fn bench_invert(c: &mut Criterion) {
    ...
    c.sample_size(10).warm_up_time(Duration::from_millis(1500)).measurement_time(Duration::from_secs(3));

    for size in SAMPLES {
        let mut image = Image::new(size, size, Rgb::white());

        c.bench_function(
            format!("invert {0}x{0} RGB", size).as_str(),
            |b| b.iter(|| image.invert()),
        );

        let mut image = Image::new(size, size, L(0));

        c.bench_function(
            format!("invert {0}x{0} L", size).as_str(),
            |b| b.iter(|| image.invert()),
        );
    }
