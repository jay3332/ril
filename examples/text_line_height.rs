...
fn main() -> ril::Result<()> {
    let font = Font::open("./examples/assets/Arial.ttf", 24.0)?;

    let mut image = Image::new(200, 200, Rgba::black());

    let (x, y) = image.center();
    let layout = TextLayout::new()
        .centered() // Shorthand for centering horizontally and vertically
        .with_wrap(image.width(), WrapStyle::Word) // Wrap the text such that it doesn't overflow the image
    ...
    image.draw(&layout);
    image.save_inferred("examples/out/text_line_height.png")?;
